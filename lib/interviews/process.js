...
var isDocx = false; // check input and output type is docx
        ...
        for (var prop in master) {
          if (master.hasOwnProperty(prop)) {
            if (typeof master[prop].values !== 'undefined') {
              if(master[prop].type =="text_dropdown"){
                answers[prop] =  master[prop].values.label[[master[prop].values[prop]]];
              }else{
      ...
      function(outcome, callback) {
        ...
        fs.mkdir(base_location + out_directory, 511, function(err) {
          ...
          async.forEachSeries(deliverables, function(deliverable, callback) {
            ...
            // set isDocx = false before check input type and output type.
            isDocx = false; 
            ...
            fs.readFile(base_location + uploads_directory + input_name, 'utf8', function(err, data) {
              ...
              try {
                ...
                if(input_type == "docx" && output_type == "docx"){
                  isDocx = true;
                }
                if(isDocx == true){
				        	//Load the docx file a template  as a binary
                  var content = fs
                      .readFileSync(base_location + uploads_directory + input_name, "binary");
                  
                  var doc = new Docxtemplater(content);
                  var Template = {
                  };
                  var answersDocx = [];
                  for (var prop in locals.answers) {
                    if(prop == "TODAY"){
                      Template['date'] = locals.answers[prop];
                    }else{
                      var temp = {
                          "title": prop,
                          "content":locals.answers[prop]
                        };
                      answersDocx.push(temp);
                    }
                  }
                  Template['answers'] = answersDocx;
                  //set the templateVariables
                  doc.setData({
                      "answers":answersDocx,
                      "date":Template['date'],
                      "user":current_deliverable.input.creator.name
                  });
                  ...
                  fs.writeFileSync( base_location + out_directory + out,buf);
                   //if we get here the process exited successfully
    						  ...
    						  if (looper) {
    							...
    							current_deliverable.input.looper = false;
