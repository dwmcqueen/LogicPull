...
exports.listen = function (server, sessionStore, app) {
  ...
  io.sockets.on('connection', function (client) {
    ...
    client.on('finish', function (data) {
      ...
      var fields = data.fields;
      var qid = data.qid;
      var outputFile = models.Outputs.find({});
      outputFile = outputFile.limit(1).sort('-date');
      //TODO: sanitize the inputs, not dry, don't query the database every time for this interview ... store the variable
      models.Interviews.findOne({ id: data.interview }, function (err, doc) {
        if (err) {
          console.log(err);
                ...
                if (deliverables.length !== 0) {
                  ...
                  var callback = function (err, data) {
                    if (err) {
                      ...
                      outputFile.exec(function (err, outputFile) {
                        if (err) {
                          console.log(err);
                          throw err;
                        } 
                         run = interview.final(qid, doc.data[qid], doc.data, connected_clients[id].data.master.vars, fields, tmp_progress, deliverables,outputFile);
